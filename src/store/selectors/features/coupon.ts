 ...
 *
 * @param state
 * Implementation of memoized selectors using reselect
 * to get particular data out of store.
 */

const couponFeatureSelector = (state: TReduxState) => state.features.coupon;

export const getCouponData = createSelector(couponFeatureSelector,
  coupon => get(coupon, 'data', null)
);
...
export const getCouponValidationStates = createSelector(couponFeatureSelector,
  coupon => get(coupon, 'validationStates', null)
);

export const getIsLoading = createSelector(getCouponValidationStates,
